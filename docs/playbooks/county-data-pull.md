...
- **Wayne County MI with Detroit breakout -- Tier 1 official county bulk plus City of Detroit ArcGIS**:
  ...
  - Live Treasurer parcel lookup: `https://devpta.waynecountymi.gov/`; submit `PARCEL_ID`, `searchByAddressInd=ParcelSearch`, `PARCEL_SEARCH_MSG=`, and `frequency=on` to `https://devpta.waynecountymi.gov/Home/SearchResult`.
  - Treasurer update-frequency documentation: `https://www.waynecountymi.gov/Government/Elected-Officials/Treasurer/Making-A-Tax-Payment/Property-Tax-Listing-FAQs`; the public lookup updates once every business day.
  - Sheriff mortgage-sale context: `https://www.sheriffconnect.com/court-services/`
  - Current mortgage-foreclosure notices: `https://www.legalnews.com/Home/PublicNotices`, the statutory-notice search reached from the Sheriff's court-services page. Search Wayne County with `foreclosures=true` and a scheduled-sale range beginning on the pull date. Notice details use `https://www.legalnews.com/Home/PublicNoticesDetails/<notice-id>`.
  - Discovery method used 2026-07-16: inspected the official county annual-data page and package URL, then used Playwright because Wayne's Akamai layer blocks direct scripted downloads. The archive's `VALUES.TXT`, `NAMES.TXT`, and `LEGALS.TXT` use the published BS&A fixed-width export layout at `https://www.bsasoftware.com/Portals/0/Support/Legacy%20Application/Assessing-Equalization/exp_gen.pdf`. Detroit's official ArcGIS services were inspected for current row counts, field definitions, edit timestamps, and paginated query support. The official tax page exposed the 2026 publication PDF, which was parsed with Ghostscript and matched to current parcel keys. The live tax route was found by inspecting the Treasurer search form and proving its POST fields against real publication parcels. The legal-notice route was found by following the Sheriff's direction to statutory notices, inspecting the Legal News advanced-search POST, and proving detail-page parcel, address, legal-description, amount, publication, and sale-date fields against the current parcel roll.
  - Working method: run `scripts/leadcurate/pull_wayne_mi.py` for the Akamai-protected assessment ZIP and tax PDF; run `build_wayne_mi_canonical.py` to stream and join the fixed-width package; run `pull_detroit_open_data.py` for current Detroit parcels, the 2026 assessment, and filtered blight tickets; run `build_wayne_mi_hybrid.py` to replace annual Detroit rows with the fresher city feed; then run `process_investor_lanes.py --market wayne-mi`, `process_wayne_mi_tax_foreclosure.py`, `verify_wayne_mi_live_tax.py`, `pull_wayne_mi_preforeclosure.py`, `process_detroit_blight_pressure.py`, and `build_wayne_mi_intelligence.py`.
  - Verified 2026-07-16: the hybrid universe contains 820,726 unique parcels, including 377,830 unique Detroit parcels and 442,896 outer-Wayne parcels. The four property lanes contain 43,784 tired landlords, 29,516 office/industrial/multifamily opportunities, 24,912 out-of-state-owner parcels, and 37,940 verified-vacant parcels. The official 2026 tax publication matched 36,327 candidates; the live Treasurer pass retained 24,663 with a positive current balance and excluded 11,664 with no positive balance returned. The current statutory-notice pull matched all 263 upcoming notices to 289 unique parcels. Every full lane file has one row per parcel and zero duplicates.
  ...
  - Live-tax rule: the annual foreclosure PDF is only a candidate universe. Run every candidate through `verify_wayne_mi_live_tax.py`; the final lane may contain only parcels for which the Treasurer returns a positive current balance. The resumable JSONL checkpoint preserves every lookup and retries request failures. Store the Treasurer's exact business-day validity date and keep no-result parcels in the audit file, not the deliverable. The verified July 16 run checked all 36,327 candidates with zero failures and zero unchecked parcels. Its 24,663 current records carry $147,242,389.55 in positive balances, a $3,727.43 median, year-level statuses, and returned tax years from 2009 through 2025, all valid as of July 15, 2026.
  - Pre-foreclosure matching rule: use published parcel or tax IDs first, then current official legal descriptions, exact address plus municipality, countywide unique address, unique owner, and only unambiguous house-number/street fallback. One notice can cover multiple parcels, so expand every resolved current parcel and deduplicate the final file by parcel ID while retaining all notice IDs. A current lane includes only notices whose scheduled sale has not passed. After the stated same-day sale time, set `--sale-date-from` to the next calendar day instead of leaving same-day rows in the delivery. The script now applies this 11:00 a.m. Detroit-time cutoff automatically when no override is supplied. The verified July 16 run excluded elapsed same-day sales, matched all 263 remaining notices to 289 current parcel rows, and produced an actual sale-date range of July 21 through September 24, 2026. Refresh on delivery day because cure, postponement, cancellation, and bankruptcy can change the event after publication.
  ...
  - Market-viability gate: a nearby or overlooked market must clear more than a population or retail screen. Run `build_market_viability.py` against the current official parcel universe and require an established improved-residential base, recent recorded residential transfers, meaningful PRE or homestead coverage, and a nonzero official assessed-value base. For Taylor, Lincoln Park, Southgate, Allen Park, and Wyandotte, the verified result is 66,316 improved residential parcels, 2,670 improved-residential transfers dated 2025 or later, 82.0% PRE proxy coverage, and a $91,500 median official residential assessment. All five cities pass. This is proof of an active property base, not a promise of buyer demand, resale, or appreciation.
  - Bundle verification: run `verify_delivery_bundle.py --processed-dir <market-date-dir> --output <market-date-dir>/delivery-verification.json` before publishing counts. It streams every referenced full CSV, verifies file-to-metadata row counts, parcel keys, duplicate counts, source URL/date/status, and writes SHA-256 hashes. Wayne's final 2026-07-16 manifest reports seven verified lanes, zero snapshot-only lanes, zero unavailable lanes, zero failed lanes, 243,850 full-file rows, and zero duplicate parcel IDs within lanes.

### Dollar Leads 19-lane expansion sources verified 2026-07-19

- **Fulton County GA full parcel roll:** `https://services1.arcgis.com/AQDHTHDrZzfsFsB5/arcgis/rest/services/Tax_Parcels_2025/FeatureServer/0`. Pull the complete Object ID list, query in chunks, and require the returned-ID set to equal the advertised-ID set. The verified pull returned 369,847 unique parcels. Do not reuse the former Atlanta-only parcel layer as Fulton County coverage. The separate current Sheriff sale PDF at `https://www.fultoncountyga.gov/inside-fulton-county/fulton-county-departments/sheriff/sheriff-sales` is image-based; download the current sale PDF, OCR it, parse parcel IDs, and join them to the full county roll. The August 4, 2026 notice produced 152 unique current parcel matches. Tax debt, recorded tax lien, and tax sale may share the same rows only because the official levy-sale notice proves all three statuses. It does not prove a separate mortgage pre-foreclosure lane.
- **Shelby County TN full parcel roll:** `https://scgis.shelbycountytn.gov/serverhigh/rest/services/Parcel/CERTParcel/MapServer/0`. Pull by complete Object ID list and reject a partial response. The verified pull returned 353,328 unique nonblank parcels after removing six blank/duplicate source keys. Event sources: current Trustee tax sale `https://scgpublic.s3.amazonaws.com/TaxSaleExtract.csv`; code cases `https://datamidsouth.opendatasoft.com/`; building/demolition permits from the same official Data Midsouth catalog. Keep only the current open code statuses and current Issued/Active/Open permits. Blight is a separate subset requiring demolition, condemnation, vacancy, board/secure, dangerous, or structural evidence. Verified counts were 1,980 tax-sale/tax-debt parcels, 7,631 code parcels, 2,875 active permits, and 821 blight-pressure parcels.
- **Dallas County TX current tax-sale source:** `https://www.dallascounty.org/Assets/uploads/docs/public-works/StruckListWorking_2025_3-3-2026.pdf`. The March 2026 struck-off resale list contains only five current DCAD-matched properties, so it is verified but below the Dollar Leads 50-record minimum. Current foreclosure and recorder events moved to the County Clerk's `publicsearch.us` application after February 24, 2026; do not substitute the older downloadable extracts. The official code and permit downloads sampled in this pass contained 2018 and 2019-2020 records, respectively, and are not current-sale sources.
- **Wayne County MI secondary event lanes:** the live Treasurer verification described above yielded 24,663 positive balances valid July 15, 2026; publish that verified set separately as recorded tax liens when the source explicitly identifies the balance as a delinquent tax lien. The refreshed Legal News notice pull returned 274 future notices and matched 299 current parcels. The current unpaid Detroit blight-ticket source contains 82,746 unique parcels and supports a separate code-violations lane as well as the narrower existing blight-pressure product.
- **Cook County IL Chicago event subset:** active building permits `https://data.cityofchicago.org/d/ydr8-5enu`; open building violations `https://data.cityofchicago.org/d/22u3-xenr`. Match an event only when its published PIN10 resolves to one current Cook parcel or its exact normalized Chicago address resolves to one current parcel. Never expand one address to every condominium PIN. Keep permit status ACTIVE with an issue date in the trailing 12 months and violation status OPEN. The corrected 2026-07-19 join produced 13,162 permit parcels, 90,560 code parcels, and a 37,000-parcel blight subset with explicit vacancy, abandonment, unsafe-structure, demolition, boarding, dangerous-building, structural, or fire-damage language.
- **Massachusetts statewide Land Court events:** `https://www.mass.gov/lists/land-court-masscourts-reports`. The nightly Tax Lien report produced 65 recent cases, of which 11 exact municipality+address matches joined to the statewide parcel roll. The nightly Servicemembers report produced 728 cases and 531 exact parcel matches. Label Servicemembers matches as a pre-foreclosure signal, never as a foreclosure judgment. The 11 tax-title matches are verified but below the store minimum.
- **Mecklenburg County NC code and blight:** open/new cases `https://gis.charlottenc.gov/arcgis/rest/services/HNS/CodeEnforcementCasesAll/MapServer/0`; demolition orders `https://gis.charlottenc.gov/arcgis/rest/services/HNS/CodeEnforcementOrderstoDemolish/MapServer/0`. Use `returnIdsOnly`, fetch every Object ID in chunks, and join the published `ParcelId` directly to the current parcel roll. The 2026-07-19 pull returned 3,784 Open/New events and matched 3,256 current parcels. Explicit blight language plus the separate demolition orders matched 398 parcels. Charlotte municipal liens come from `https://data.charlottenc.gov/api/download/v1/items/107e93008cbc4430ad2a3afafa839a24/csv?layers=0`; the verified current join contains 460 unique parcels and publishes under the standard `property-liens` lane.
...
- **Massachusetts statewide -- Tier 1 official MassGIS ArcGIS REST**:
  ...
  - Fresh pull verified 2026-08-25: the live parcel layer reported 2,559,286 rows and an exact `dataLastEditDate` of `2026-08-17T17:50:55.804000+00:00`. `pull_massgis_statewide.py` fetched all rows, retained 2,558,988 unique parcels, and removed 298 duplicates. For customer packages that require recent assessor facts, filter the municipal `FY` field separately from the ArcGIS service edit date. The DeShawn package used only FY2025, FY2026, and FY2027 parcels.
  - Fresh Land Court method verified 2026-08-25: the official report page remains `https://www.mass.gov/lists/land-court-masscourts-reports`, with direct documents at `https://www.mass.gov/doc/servicemember-cases/download` and `https://www.mass.gov/doc/tax-lien-cases/download`. Mass.gov returned HTTP 403 to direct `curl` and Python requests from the working environment. The successful method was to open the official report page in a real Playwright browser, fetch each PDF in the browser session, save it locally, and copy it to the VPS. `build_massachusetts_event_lanes.py` then performed exact municipality and address matching. The fresh reports produced 596 unique pre-foreclosure parcel matches and 4 unique tax-title parcel matches before customer-package ownership, core-field, and fiscal-year filters.
  - Bulk probate limitation verified 2026-08-25: the official Probate and Family Court public-record FAQ is `https://www.mass.gov/info-details/probate-and-family-court-access-to-public-court-records-frequently-asked-questions`. Estates and Administrations are publicly searchable by name, case number, or case type, but no verified statewide bulk probate filing report was found. Do not substitute owner-name keywords for a probate event.
