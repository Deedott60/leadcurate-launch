...
## 6.5. Editing & publishing docs/*.html pages (GitHub Pages)

Repo lives locally at `/root/leadcurate-launch` (remote `origin` = `github.com/Deedott60/leadcurate-launch`, already authenticated via a token embedded in the remote URL — never ask Derrick for git credentials, and Derrick never touches the VPS or git himself, per §11).

**Workflow — always in this order:**
1. `cd /root/leadcurate-launch && git fetch origin && git pull origin main` — REQUIRED before editing, every time. Claude/Codex push directly to `main` outside this session, so local can be behind. (Confirmed 2026-07-03: local was behind origin on `docs/property-numbers/index.html` — pulling first avoided clobbering a newer version that had already landed.)
2. Target file is under `docs/<page-slug>/index.html` (e.g. `docs/property-numbers/index.html`, `docs/data-audit/index.html`). If the user uploads/pastes an HTML file to compare against, `diff` it against the repo copy first to see exactly what already changed before editing blind.
3. For the phone/PWA operator app, the file is `docs/command/index.html`. If Derrick says a link exists in the desktop/web version but not in the phone app, add the missing link directly to the sidebar/nav or command palette, not only to a deeper page card. The phone app hides some topbar controls and users rely on the sidebar.
4. Edit with a targeted find/replace, not a full rewrite — these are long single-file HTML docs; a full-file rewrite risks losing unrelated content.
5. `git add <file> && git commit -m "<what changed>" && git push origin main`.
6. **Verify live, don't assume.** GitHub Pages redeploy takes roughly 30–90 seconds after push. `sleep 40` then `curl -s https://leadcurate.com/<page-slug>/ | grep -o "<new text>"`; retry once more if empty. Only report the page as updated once the new text is confirmed present (and old text confirmed absent) in the live curl output — not from the local file diff alone.
7. For mobile app/PWA cache issues, give Derrick a cache-busted URL like `https://leadcurate.com/command/?v=<commit>` after live verification, and tell him to close/reopen the app if the installed PWA still shows the old cached shell.

**Customer-facing edit pitfalls:**
- Internal-only sections (a "Method" / "How these numbers were derived" methodology block, QA notes, internal version dates) do not belong on pages meant to go external (Facebook, prospects, cold outreach). Strip them whenever the user says something like "I want to send this to anybody."
- When rewriting a customer-facing headline or any copy on these pages, apply the §4 brand-voice rules (no "cheap/save/starting at just", frame around fit/quality/exclusivity) even for a one-line tweak — this is a customer-touching surface same as intake/quote copy.
- Derrick flagged em dashes (`—`) and over-polished phrases as AI-looking in customer-facing LeadCurate pages. Prefer plain punctuation and plainer labels: e.g. replace "The deepest comp data in the inventory" with "Deep sales-comp coverage" and use hyphens/periods/commas instead of long dashes.
- For public share links, give Derrick the normal `leadcurate.com/<slug>/` URL, and if Facebook/browser caching may show the old copy, also provide a cache-busted verification URL like `?v=<commit>`.

Supporting reference: `references/customer-facing-audit-cleanup-2026-07-03.md`.
...
7. **Two-mode communication.** Preview Audit = sales (redacted, charts, "Reserve Your County" CTA). Delivery Audit = post-paid (full data, XLSX attached, "Your file is attached"). Both share the same brand experience; neither leaks internal jargon. **NAMING RULE (locked 2026-07-03):** the word "Sample" reads as "fake/demo" to a real prospect and must NEVER appear in customer-facing copy — customer-facing label is "Preview Audit." The internal system/API parameter name (`mode: "sample"` in send-delivery) stays as-is, that's plumbing, not something a customer reads. Only the words a customer actually sees changed.
...
> **STALE NOTICE (2026-07-08):** `send-delivery` got a full rebuild today, now at v13. It's ONE generic renderer now, not fixed required fields — read `supabase/functions/send-delivery/index.ts` directly (`renderSample`/`renderDelivery`/`deriveNumbers`/`extraColumns`) before assuming the field list above is current. Also: signs as "The LeadCurate Team" now, never a personal name. Zero em dashes allowed in anything customer-facing — grep for `—` before calling copy done, this is a hard rule now (see `docs/AGENT-OPERATING-RULES.md`).

---

## B2B outreach workflow — the actual repeatable pipeline (2026-07-08)

Derrick's ask: stop hunting for prospect emails one at a time, Danny should be able to research, draft, and (after approval) send outreach autonomously, using tools already available — `browser`, `email/himalaya`, `social-media/xurl`, `terminal`. This is orchestration of existing tools, not new infrastructure.

**No native Stripe tool exists, and none is needed.** Payment happens through the already-deployed `payment-confirmation` Supabase Edge Function (Codex, 2026-07-07). Call it with `terminal`/`curl` like any other API. Live Stripe keys are still pending Derrick's provider pick — verify current state in `activity_feed`/`docs/CURRENT-HANDOFF.md` before promising a customer a specific payment method.

1. **Research (fully automated).** For a B2B lane (Asset Locator → collection attorneys; Code Violations → restoration/roofing contractors) and a market we have data for, use `browser` to find real companies: firm name, real contact email, social handle where findable (`xurl` for X/LinkedIn lookups). Write to the `prospects` table, dedupe by email, never insert test rows.
2. **Draft (fully automated).** Use the locked messaging in `docs/outreach/b2b-lanes-2026-07-06.md`, personalized with the real prospect + one real stat pulled from the actual sample/audit page (`docs/sample-deliveries/`). Never fabricate a stat.
3. **Queue for approval (manual-trigger rule still applies).** Post the draft to `activity_feed` as `conf:role` targeted at `derrick`, or into `messages` as `pending`. Do not send without Derrick's go — this protects domain sending reputation and catches mistakes before they reach a real company. If Derrick explicitly authorizes skipping this for a batch, that needs to be in writing (a `conf:role` post), not assumed.
4. **Send (after go-ahead).** Via Hostinger Mail API (same pattern as `send-delivery`, `HOSTINGER_MAIL_TOKEN` via `get_app_secret`) or `email/himalaya` if wired to the same mailbox. Log to `activity_feed`, update prospect status.
5. **Reply handling.** Answer factual questions about what's in the file using the "What's in your file" pattern from the audit pages. Offer the public sample/audit link freely. Always check with Derrick before sending anything beyond that — the full paid file only goes out after payment confirms.
6. **Payment.** Call `payment-confirmation` via `terminal`/`curl` to create the order/payment record. Confirm which mode is actually live (manual Cash App/Zelle vs. Stripe Payment Link) before telling a customer what payment methods are available.
